...
const Services = () => {
  const [isSubscriptionModalOpen,setSubscriptionModal] = useState(false)
   ...
   const handleSubscription = async(subscriptionId:string)=>{
    ...
    const handleGoProButton = async()=>{
      try {
        if(user === null){
          showErrorToast('Login to proceed')
          return 
        }
  ...
  return (
    ...
    <section className="w-full py-16 px-4 md:px-8" style={{backgroundImage:"url('/white-bg.jpg')"}}>
        ...
        <Button onClick={handleGoProButton} className="mt-4 bg-[#FFC436] text-black px-6 py-3 rounded-lg hover:bg-[#0f0f0f] hover:text-[#FFC436]" >
          Go Pro
        </Button>
      </div>
