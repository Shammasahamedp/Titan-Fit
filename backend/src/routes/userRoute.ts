...
userRouter.get('/profile',jwtTokenVerify(['user']),checkIfUserBlocked,(req:Request,res:Response)=>userController.getProfile(req,res)).put('/profile',jwtTokenVerify(['user']),checkIfUserBlocked,validate(userProfileEditSchema),(req:Request,res:Response)=>userController.editProfile(req,res))
