...
export class UserService implements IUserService {
  ...
  async editUserProfile(
    ...
    try {
      const editedProfileData = await this.userRepository.editUserProfile(
        userId,
        userProfileData
      );
      if (!editedProfileData) {
        throw new AppError(
          "failed to edit user data , new edited user not found",
          404
        );
      }
  ...
  async getApprovedTrainers(
    ...
    search: string,
    date: string
  ): Promise<{ trainers: ITrainerDocument[]; total: number } | null> {
    try {
  ...
  async getSingleApprovedTrainer(
    trainerId: string
  ): Promise<{
    ...
    try {
      const trainer = await this.trainerRepository.getApprovedTrainer(
        trainerId
      );
      if (!trainer) {
        throw new AppError(trainerMessages.APPROVED_TRAINERS_NOT_FOUND, 404);
      ...
      const availability = await this.trainerAvailability.findOne({
      ...
      throw new AppError(
        "something went wrong while fetching trainer details",
        500
      );
    }
  }

  async bookASessionWithTrainer(
    trainerId: string,
    userId: string,
    date: string,
    startTime: string
    ...
    try {
      ...
      if (user?.subscription) {
        ...
        const alreadyBooked =
          await this.trainerAvailability.findUserSlotInAnyTrainerInADate(
            user._id,
            date
          );
          console.log('alreadybooked',alreadyBooked)
        if (alreadyBooked) {
          console.log('insidealreadybooked')
          ...
          if (isTomorrow) {
            throw new AppError(
              "cannot change session booked for tomorrow",
              400
            );
          }
         
          if(alreadyBooked.trainerId.toString() !== trainerId){
                throw new AppError('You have already booked the session at this date with another trainer',400)
          }
          const changed = await this.trainerAvailability.changeUserSlotOnDate(
            trainerId,
            user._id,
            date,
            startTime
          ...
          const availability = await this.trainerAvailability.bookASession(
            trainerRefId,
            userRefId,
            date,
            startTime
          ...
          if (!availability) {
            throw new AppError(
              availabilityMessages.AVAILABILITY_NOT_FOUND,
              404
            );
          }
        }
      ...
      throw new AppError(
        "something went wrong while booking a training session",
        500
      );
    }
  }
  ...
  async cancelTrainingSession(trainerId: string, userId: string, date: string, startTime: string): Promise<boolean> {
      try {
       ...
       const doc= await this.trainerAvailability.cancelTrainerBookedSession(trainerId,userId,date,startTime)
       if(!doc){
         throw 'something went wrong'
       }
       ...
       return true
        ...
        throw new AppError('something went wrong while cancel the training session',500)      }
  }
  ...
  async updateExpiredSubscription(): Promise<void> {
    try {
        ...
        for (let user of activeSubscribedUsers) {
          ...
          if (user.subscription && Array.isArray(user.subscription)) {
            user.subscription.forEach((sub) => {
              if (
                (sub.status === "active" && sub.endDate < today) ||
      ...
      throw new AppError(
        "something went wrong while updating the subscription status",
        500
      );
    }
  }
